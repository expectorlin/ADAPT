...
class VisionEncoder(nn.Module):
    def __init__(self, vision_size, config):
        ...
        if args.features == 'clip':
            self.visn_fc_pre = nn.Linear(512 + 128, feat_dim)

    def forward(self, visn_input):
        feats = visn_input

        if args.features == 'clip':
            feats = self.visn_fc_pre(feats)
...
class VLNBert(BertPreTrainedModel):
    def __init__(self, config):
        ...
        if args.ADAPT:
            self.txt_enc = nn.Sequential(nn.Linear(512, 768),
                                                nn.Dropout(0.1))
            self.img_enc = nn.Sequential(nn.Linear(512, 768),
                                                nn.Dropout(0.1))
            self.action_enc = nn.Sequential(nn.Linear(768*2, 768),
                                                nn.Dropout(0.1))
    ...
    def forward(self, mode, input_ids, token_type_ids=None,
        attention_mask=None, lang_mask=None, vis_mask=None, position_ids=None, head_mask=None,
                img_feats=None, prompt_mask_set=None,
                txt_sub_prompt_set=None, img_sub_prompt_set=None):
        ...
        if mode == 'language':
            ...
            if txt_sub_prompt_set is not None and img_sub_prompt_set is not None:
                extended_prompt_mask = prompt_mask_set.unsqueeze(1).unsqueeze(2)
                extended_prompt_mask = extended_prompt_mask.to(dtype=next(self.parameters()).dtype)
                extended_prompt_mask = (1.0 - extended_prompt_mask) * -10000.0
                prompt_mask = extended_prompt_mask.cuda()

                text_mask = torch.cat([text_mask, prompt_mask], dim=3)
                txt_sub_prompt_feat = self.txt_enc(txt_sub_prompt_set)
                img_sub_prompt_feat = self.img_enc(img_sub_prompt_set)
                prompt_feat = self.action_enc(torch.cat([txt_sub_prompt_feat, img_sub_prompt_feat], dim=-1))
                lang_output = torch.cat([lang_output, prompt_feat], dim=1)
            ...
            if prompt_feat is not None and prompt_mask_set is not None:
                language_attention_scores = language_attention_scores[:, :,
                                            :language_attention_scores.size(2) - prompt_feat.size(1)]
                prompt_attention_scores = language_attention_scores[:, :,
                                       language_attention_scores.size(2) - prompt_feat.size(1):]

                prompt_scores = prompt_attention_scores.mean(dim=1)
                prompt_probs = nn.Softmax(dim=-1)(prompt_scores.clone()).unsqueeze(-1)
                attended_txt_sub_prompt_feat = (prompt_probs * txt_sub_prompt_feat).sum(1)
                attended_img_sub_prompt_feat = (prompt_probs * img_sub_prompt_feat).sum(1)
            ...
            if prompt_feat is not None and prompt_mask_set is not None:
                return pooled_output, visual_action_scores, attended_language, attended_visual, attended_txt_sub_prompt_feat, attended_img_sub_prompt_feat, txt_sub_prompt_feat, img_sub_prompt_feat
            else:
