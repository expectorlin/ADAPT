...
''' train the listener '''
def train(train_env, tok, n_iters, log_every=2000, val_envs={}, aug_env=None, feat_dict=None):
    writer = SummaryWriter(log_dir=log_dir)
    listner = Seq2SeqAgent(train_env, "", tok, args.maxAction, feat_dict)
    ...
    while idx < start_iter + n_iters:
        ...
        if args.ADAPT:
            consistency_loss = sum(listner.logs['consistency_loss']) / max(len(listner.logs['consistency_loss']), 1)
            writer.add_scalar("loss/consistency_loss", consistency_loss, idx)
            alignment_loss = sum(listner.logs['alignment_loss']) / max(len(listner.logs['alignment_loss']), 1)
            writer.add_scalar("loss/alignment_loss", alignment_loss, idx)
...
def valid(train_env, tok, val_envs={},
                      feat_dict=None):
    agent = Seq2SeqAgent(train_env, "", tok, args.maxAction, feat_dict)
...
def train_val(test_only=False):
    ...
    if args.ADAPT and args.features == 'places365':
        feat_dict_clip = read_img_features(CLIP_FEATURES, test_only=test_only)
